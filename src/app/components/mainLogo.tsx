...
const MainLogo: FC = () => {
    ...
    const render = (time: number) => {
      ...
      for (let y = 0; y < RENDER_TEXT_LINES.length; y++) {
        ...
        for (let x = 0; x < line.length; x++) {
          ...
          if (animationIndex == 0) {
            //0: final shiny wave
            const waveDistance = Math.abs(Math.floor(slowTime % (TEXT_WIDTH + 25) - 20) - (x - y))
            ...
            //3: sine wave
            const sineDistance = (Math.sin(x / 10 + slowTime / 5) * TEXT_HEIGHT / 2 + TEXT_HEIGHT / 2) - y
            ...
            const lerpH = lerpAngle(0, x / TEXT_WIDTH * 360 - slowTime * 10, sineLerp)
  ...
  return (
    <canvas className={css.container} ref={canvasRef}></canvas>
  )
}
